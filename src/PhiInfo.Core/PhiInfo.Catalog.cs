...
internal sealed class ByteReader(byte[] data)
{
    ...
    public int ReadInt()
    {
        var value = BitConverter.ToInt32(_data, _pos);
        _pos += 4;
        return value;
    }
}
...
public readonly struct CatalogKey
{
    ...
    {
        return new CatalogKey(type, value, null);
    }

    public static CatalogKey FromByte(byte value)
    {
        return new CatalogKey(CatalogKeyType.Byte, null, value);
    }

    public override string ToString()
    {
        return StringValue ?? ByteValue?.ToString() ?? string.Empty;
    }
}
...
public readonly struct CatalogValue
{
    ...
    {
        return new CatalogValue(true, value, null);
    }

    public static CatalogValue FromResolved(CatalogKey key)
    {
        return new CatalogValue(false, 0, key);
    }
}
...
public sealed class CatalogParser
{
        ...
        byte[] entryData)
    {
        ...
        var bucketCount = reader.ReadInt();

        for (var i = 0; i < bucketCount; i++)
        {
            var keyPos = reader.ReadInt();
            ...
            switch (keyType)
            {
                ...
                case CatalogKeyType.UnicodeString:
                    var length = BitConverter.ToInt32(keyData, keyPos);
                    keyPos += 4;
            ...
            var entryPos = reader.ReadInt();
            for (var j = 1; j < entryCount; j++)
                reader.ReadInt();
            var entryStart = 4 + 28 * entryPos;
            var raw = (ushort)(entryData[entryStart + 8] ^ (entryData[entryStart + 9] << 8));
